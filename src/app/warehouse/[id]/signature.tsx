'use client'
import { documentTranslateStatus, translateStatus } from '@/app/warehouse/[id]/data'
...
export default function WarehouseSignatureLayout({ data, id }: { data: SignatureWarehouse; id: string }) {
                    ...
                    <>
                        ...
                        <div className="md:w-1/3 w-full">
                            ...
                            <p className="text-gray-900">
                                {data.model.deliveredDate
                                    ? moment(data.model.deliveredDate).format('DD MMM YYYY')
                                    : '-'}
                            </p>
                        </div>
                        <div className="md:w-1/3 w-full">
                            <h5 className="font-bold text-base text-gray-700">Status</h5>
                            <p className="text-gray-900">{documentTranslateStatus(data.model.currentStatus)}</p>
                        </div>
                ...
                return (
                    <>
                        ...
                        <div className="md:w-1/3 w-full">
                            ...
                            <p className="text-gray-900">
                                {moment(data.model.expectedDeliveryDate).format('DD MMM YYYY')}
                            </p>
                        </div>
                        <div className="md:w-1/3 w-full">
                ...
                return (
                    <>
                        <div className="md:w-1/3 w-full">
                            <h5 className="font-bold text-base text-gray-700">Tanggal Diterima</h5>
                            <p className="text-gray-900">
                                {moment(data.model.receivedDate).format('DD MMM YYYY HH:mm')}
                            </p>
                        </div>
                        <div className="md:w-1/3 w-full">
                            <h5 className="font-bold text-base text-gray-700">Status</h5>
                            <p className="text-gray-900">{translateStatus(data.model.status)}</p>
                        </div>
                        {data.model.description && (
                            <div className="w-full">
                                <h5 className="font-bold text-base text-gray-700">Deskripsi</h5>
    ...
    const approvedApprovers = data.approvers.filter(approver => approver.status.toLowerCase() !== 'pending')

    return (
        <>
            <div
                className="bg-white px-3 py-6 md:px-12 md:py-16 rounded relative overflow-hidden"
                style={{ minHeight: '50%' }}
            >
                {/* Watermark */}
                <div
                    className="absolute inset-0 pointer-events-none opacity-10 z-0 flex items-center justify-center"
                    ...
                    <div className="my-4">
                        <div className="flex justify-between flex-wrap gap-y-3">
                            <div className="md:w-1/3 w-full">
                                ...
                                <p className="text-gray-900">{translateStatus(data.status)}</p>
                            </div>
                            <div className="md:w-1/3 w-full">
                                ...
                                <div key={index} className="mb-6 last:mb-0">
                                    <div className="shadow-md p-4 rounded-lg mb-3">
                                        <div className="flex justify-between flex-wrap gap-y-3">
                                            ...
                                            <div className="md:w-1/3 w-full">
                                                <h5 className="font-bold text-base text-gray-700">
                                                    Status Persetujuan
                                                </h5>
                                                <p className="text-gray-900">{translateStatus(approver.status)}</p>
                                            </div>
                                        </div>
                                    </div>
                                    {index < approvedApprovers.length - 1 && <hr className="my-3 border-gray-300" />}
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div className="my-4">
                        ...
                        <div className="flex justify-between flex-wrap gap-y-3">
                            <div className="md:w-1/3 w-full">
                                ...
                                <p className="text-gray-900">
                                    {moment(data.model.createdAt).format('DD MMM YYYY HH:mm')}
                                </p>
                            </div>
                            <div className="md:w-1/3 w-full">
